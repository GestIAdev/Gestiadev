...
interface ArmeriaBarchafyDetalleProps {
  setActiveView: (view: string) => void;
}

const ArmeriaBarchafyDetalle = ({ setActiveView }: ArmeriaBarchafyDetalleProps) => {
  const [isReadyToMount, setIsReadyToMount] = useState(false);
  const [featureIndex, setFeatureIndex] = useState(0);
  const demoUrl = 'https://barchafy.gestiadev.workers.dev/';

  // ARRAY DE DOCTRINA - Features del Visionario
  const features = [
    "Cartas Coleccionables",
    "API de Cocteles",
    "Motor de Eventos Dinámicos"
  ];
    ...
    <div className="fixed inset-0 z-[100] bg-[#0A0A0A]">
      ...
      {isReadyToMount ? (
        ...
        <iframe
          ...
          title="Demo Viva: BarChafy - Sistema de Gestión de Eventos y Cocteles"
          allow="fullscreen"
        />
      ) : (
          ...
          <div className="text-center">
            ...
            <div className="space-y-2">
              <p className="text-gris-neutro/70 font-plex-mono text-sm">
                [DEMO VIVA] - GESTIÓN DE EVENTOS ACTIVA
              </p>
              <p className="text-menta/60 font-plex-mono text-xs animate-pulse">
                Conectando a barchafy.gestiadev.workers.dev...
              </p>
            </div>
          </div>
        </div>
...
export default ArmeriaBarchafyDetalle;